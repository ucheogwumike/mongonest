import { Test, TestingModule } from '@nestjs/testing';
import { CreateUserDto } from './dto/create-user.dto';
import { User } from './schemas/user.schema';
import { UserController } from './user.controller';
import { UserService } from './user.service';

describe('UserController', () => {
  let controller: UserController;
  const mockUserService = {
    create: jest.fn(),
    findUser: jest.fn(),
    avatarToPicture: jest.fn(),
    deleteAvatar: jest.fn(),
  };

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [UserController],
      providers: [
        {
          provide: UserService,
          useValue: mockUserService,
        },
      ],
    }).compile();

    controller = module.get<UserController>(UserController);
  });

  it('create => should create a new user by a given data', async () => {
    // arrange
    const createUserDto = {
      first_name: 'Chadwick',
      last_name: 'Boseman',
      email: 'chadwickboseman@email.com',
    } as CreateUserDto;

    const user = {
      first_name: 'Chadwick',
      last_name: 'Boseman',
      email: 'chadwickboseman@email.com',
    } as User;

    jest.spyOn(mockUserService, 'create').mockReturnValue(user);

    // act
    const result = await controller.create(createUserDto);

    // assert
    expect(mockUserService.create).toBeCalled();
    expect(mockUserService.create).toBeCalledWith(createUserDto);

    expect(result).toEqual(user);
  });

  it('findOne => should find a user on the third party api by a given id and return its data', async () => {
    //arrange
    const userid = '1';
    const user = {
      id: 1,
      email: 'george.bluth@reqres.in',
      first_name: 'George',
      last_name: 'Bluth',
      avatar: 'https://reqres.in/img/faces/1-image.jpg',
    };

    jest.spyOn(mockUserService, 'findUser').mockReturnValue(user);

    //act
    const result = await controller.findUser(userid);

    expect(result).toEqual(user);
    expect(mockUserService.findUser).toBeCalled();
    expect(mockUserService.findUser).toBeCalledWith(userid);
  });

  it('avatar => should generate a base 64 string', async () => {
    //arrange
    const userid = '1';
    const user = {
      data: '',
    };

    jest.spyOn(mockUserService, 'avatarToPicture').mockReturnValue(user);

    //act
    const result = await controller.avatarToPicture(userid);

    expect(result).toEqual(user);
    expect(mockUserService.avatarToPicture).toBeCalled();
    expect(mockUserService.avatarToPicture).toBeCalledWith(userid);
  });

  it('avatar => should remove a base 64 string and delete file', async () => {
    //arrange
    const userid = '3';
    const user = {
      message: 'avatar deleted successfully',
    };

    jest.spyOn(mockUserService, 'deleteAvatar').mockReturnValue(user);

    //act
    const result = await controller.deleteAvatar(userid);

    expect(result).toEqual(user);
    expect(mockUserService.deleteAvatar).toBeCalled();
    expect(mockUserService.deleteAvatar).toBeCalledWith(userid);
  });
});
